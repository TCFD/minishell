...
/*   Updated: 2022/12/16 16:01:26 by tboldrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	write_hexa(char *base, unsigned int integer)
{
	int		idx;
	int		len_integer;
	char	*hexa;
	...
	hexa[idx] = '\0';
	len_integer = write_str_reverse(hexa);
	free(hexa);
	return (len_integer);
}
...
int	write_addr(void *pointeur, char *base)
{
	...
	if ((long long int)pointeur == -1)
		return (write_str("0xffffffffffffffff"));
