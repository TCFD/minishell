...
{
	if (red->list)
		ft_lstadd_back(&red->list, ft_lstnew(ft_strdup_protect(tb[i]), ty[i]));
	else
		red->list = ft_lstnew(ft_strdup_protect(tb[i]), ty[i]);
}
...
void	temp_heredoc(char *str, char **random_adress)
{
	...
	*random_adress = ft_join(ft_strdup_protect("/tmp/"), *random_adress);
